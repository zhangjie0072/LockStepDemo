...
namespace IM
{
    ...
    {
        public const int BIG_FACTOR = 1000000; 
        ...
        public static Number QTR_CIRCLE = new Number(90);
        public static Number HALF_CIRCLE = new Number(180);
        public static Number CIRCLE = new Number(360);
        ...
        public static void CheckRange(Vector2 vec)
        {
            CheckLengthRange(vec.x, "Vector2.x");
            CheckLengthRange(vec.y, "Vector2.y");
        }
        ...
        public static Number DeltaAngle(Number current, Number target)
        {
            Number delta = target - current;
            if (delta > Math.HALF_CIRCLE)
                delta -= Math.CIRCLE;
            else if (delta < -Math.HALF_CIRCLE)
                delta += Math.CIRCLE;
            return delta;
        }

        public static Number Lerp(Number from, Number to, Number t)
        {
            t = Math.Clamp(t, Number.zero, Number.one);
            Number n = from * (Number.one - t) + to * t;
            return n;
        }

        public static Number LerpAngle(Number from, Number to, Number t)
        {
            t = Math.Clamp(t, Number.zero, Number.one);
            Number delta = Math.DeltaAngle(from, to);
            return from + delta * t;
        }

        public static Number Clamp01(Number x)
        {
            return Clamp(x, Number.zero, Number.one);
        }
