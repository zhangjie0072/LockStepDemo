	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : COLOR
			{
				...
				col = orgCol * i.color;
				fixed greyFactor = dot(orgCol, grey);
				//col.rgb = lerp( lerp(float4(greyFactor,greyFactor,greyFactor,greyFactor), orgCol, 0.1).xxx, col.rgb, saturate( (i.color.r - 0.001) * 1000 ) );
