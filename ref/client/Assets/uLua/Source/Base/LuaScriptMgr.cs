...
public class LuaScriptMgr
{
    ...
    public LuaState lua;
	public Dictionary<string, object[]> fileList = null;
    ...
    public LuaScriptMgr()
    {
		...
		fileList = new Dictionary<string, object[]>();
    ...
    {
		object[] ret = null;
        if (!fileList.TryGetValue(fileName, out ret))
        {
			ret = lua.DoFile(fileName, null);
			fileList.Add(fileName, ret);
            return ret;                  
        }
    ...
    public byte[] Loader(string name)
    {
        ...
        try
        {
            if ( Application.isEditor)	//移动平台下，Lua脚本暂时放在Resources里面，由Unity管理
            {
            ...
            else
            {
                ...
                TextAsset textAsset = ResourceLoadManager.Instance.GetLuaResource(name) as TextAsset;
                str = textAsset.bytes;
            }
        }
        catch
