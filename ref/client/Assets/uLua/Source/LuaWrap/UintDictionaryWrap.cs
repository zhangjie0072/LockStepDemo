...
public class UintDictionaryWrap
{
	public static void Register(IntPtr L)
	{
		LuaMethod[] regs = new LuaMethod[]
		{
			...
			new LuaMethod("New", _CreateUintDictionary),
		...
		LuaScriptMgr.RegisterLib(L, "UintDictionary", typeof(Dictionary<uint,uint>), regs, fields, typeof(object));
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int _CreateUintDictionary(IntPtr L)
	{
		int count = LuaDLL.lua_gettop(L);

		if (count == 0)
		{
			Dictionary<uint,uint> obj = new Dictionary<uint,uint>();
		...
		else if (count == 1 && LuaScriptMgr.CheckTypes(L, 1, typeof(int)))
		{
			...
			Dictionary<uint,uint> obj = new Dictionary<uint,uint>(arg0);
			LuaScriptMgr.PushObject(L, obj);
			return 1;
		}
		else if (count == 1 && LuaScriptMgr.CheckTypes(L, 1, typeof(IDictionary<uint,uint>)))
		{
			IDictionary<uint,uint> arg0 = (IDictionary<uint,uint>)LuaScriptMgr.GetNetObject(L, 1, typeof(IDictionary<uint,uint>));
			Dictionary<uint,uint> obj = new Dictionary<uint,uint>(arg0);
			LuaScriptMgr.PushObject(L, obj);
			return 1;
		}
		...
		else if (count == 2 && LuaScriptMgr.CheckTypes(L, 1, typeof(int), typeof(IEqualityComparer<uint>)))
		{
			...
			Dictionary<uint,uint> obj = new Dictionary<uint,uint>(arg0,arg1);
			LuaScriptMgr.PushObject(L, obj);
			return 1;
		}
		else if (count == 2 && LuaScriptMgr.CheckTypes(L, 1, typeof(IDictionary<uint,uint>), typeof(IEqualityComparer<uint>)))
		{
			IDictionary<uint,uint> arg0 = (IDictionary<uint,uint>)LuaScriptMgr.GetNetObject(L, 1, typeof(IDictionary<uint,uint>));
			IEqualityComparer<uint> arg1 = (IEqualityComparer<uint>)LuaScriptMgr.GetNetObject(L, 2, typeof(IEqualityComparer<uint>));
			Dictionary<uint,uint> obj = new Dictionary<uint,uint>(arg0,arg1);
		...
		{
			LuaDLL.luaL_error(L, "invalid arguments to method: Dictionary<uint,uint>.New");
		}

		return 0;
	}

	static Type classType = typeof(Dictionary<uint,uint>);
	...
	static int get_Count(IntPtr L)
	{
		...
		Dictionary<uint,uint> obj = (Dictionary<uint,uint>)o;
	...
	static int get_Item(IntPtr L)
	{
		...
		Dictionary<uint,uint> obj = (Dictionary<uint,uint>)LuaScriptMgr.GetNetObjectSelf(L, 1, "Dictionary<uint,uint>");
	...
	static int GetEnumerator(IntPtr L)
	{
		LuaScriptMgr.CheckArgsCount(L, 1);
		Dictionary<uint,uint> obj = (Dictionary<uint,uint>)LuaScriptMgr.GetNetObjectSelf(L, 1, "Dictionary<uint,uint>");
		Dictionary<uint,uint>.Enumerator o = obj.GetEnumerator();
		LuaScriptMgr.Push(L, o);
		return 1;
	}
}
