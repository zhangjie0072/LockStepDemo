...
public class AnimatorStateInfoWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("speed", get_speed, null),
			new LuaField("speedMultiplier", get_speedMultiplier, null),
	...
	static int get_speedMultiplier(IntPtr L)
	{
		...
		if (o == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name speedMultiplier");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index speedMultiplier on a nil value");
			}
		}

		AnimatorStateInfo obj = (AnimatorStateInfo)o;
		LuaScriptMgr.Push(L, obj.speedMultiplier);
		return 1;
	}
