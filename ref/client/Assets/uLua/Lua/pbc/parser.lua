...
function parser.parser(text,filename)
	...
	--local all = {}
	_gen_fullname(t,parser.all)
	_fix_typename(t , parser.all)
	return t
end
...
--临时的，Proto文件放在了Resources里，先加载进来，再注册buffer
function parser.register_buffer(filename, buffer, path)
	local files = {}
	local t = parser_one(buffer,filename)
	_gen_fullname(t,parser.all)
	t.name = filename
