...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		{	
			...
			Debug.Log("Adding lib paths...");
			foreach( string libpath in mod.libpaths ) {
				if (libpath.Contains("$(inherited)")) {
					Debug.Log ("not prepending a path to " + libpath);
					this.AddLibrarySearchPaths( libpath );
				} else {
					string absoluteHeaderPath = System.IO.Path.Combine( mod.path, libpath );
					this.AddLibrarySearchPaths( absoluteHeaderPath );
				}
			}
			Debug.Log("Adding frame paths...");
			foreach( string framepath in mod.framepaths ) {
				if (framepath.Contains("$(inherited)")) {
					Debug.Log ("not prepending a path to " + framepath);
					this.AddFrameworkSearchPaths( framepath );
				} else {
					string absoluteHeaderPath = System.IO.Path.Combine( mod.path, framepath );
			...
			{				
				...
				string absoluteplistPath = System.IO.Path.Combine( mod.path, mod.plist );
				if(File.Exists(absoluteplistPath))
					File.Copy(absoluteplistPath,plistPath);

//				XCPlist plist = new XCPlist (plistPath);
//				plist.Process(mod.plist);
			}
			Debug.Log("Replace appcontroller...");
			if(mod.Controller!=null)
			{
				foreach(string str in mod.Controller)
				{
					string controllerName = Path.GetFileName(str);
					string filePath = this.projectRootPath + "/Classes/"+controllerName;
					if(File.Exists(filePath))
					{
						File.Delete(filePath);
					}
					Debug.Log("replace controller file :"+filePath);
					string absoluteFilePath = System.IO.Path.Combine( mod.path, str );
					if(File.Exists(absoluteFilePath))
						File.Copy(absoluteFilePath,filePath);
				}
			}
			this.Consolidate();
		}
